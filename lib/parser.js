...
function parse(trace) {
  ...
  //clever, actually a pattern, that if matches, used to excluding leading text
  const dataPattern = /^(?:[a-z0-9]{4}:) ((?:[a-z0-9]{2} ){1,16})/;
  const dirPattern = /^(=>|<=)/;
  const filterSSLdata = / SSL data/;
  var inSSL=false;
  ...
  for (const line of traceLines) {
    ...
    if (dirMatch !== null) {
      const sslData = filterSSLdata.exec(line);
      if ( sslData != null ) {
        inSSL=true;
        continue; // filter out ^(<=)|(=>) (Recv)|(Send) SSL data
      } else {
        inSSL=false;  // this is content we want to ingest
      }
      lastDir = dirMatch[0].trim();
    }
    if ( inSSL ) {
          continue; // don't ingest SSL data; we also ignore ^== Info lines
    }
    const dataMatch = dataPattern.exec(line);
    if (dataMatch !== null) {
      // note that we're using the remainder of a matched data line
